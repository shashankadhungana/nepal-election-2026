...
def clean_df(df):
    ...
    # FIXED: Map official EC "Elected" to "Won" for final-results logic
...
def hero_metrics(df):
    ...
    if declared.empty:
        ...
        with col1:
            st.metric("🏛️ Constituencies", total_races, "0 declared")
        with col2:
...
def majority_and_pr_section(df):
    ...
    for p in fptp_counts.head(8).index:
        combined_data.append({
            "party": p,
            "FPTP": int(fptp_counts[p]),
            "PR": pr_inputs.get(p, 0),
            "Total": int(fptp_counts[p]) + pr_inputs.get(p, 0)
        })
    combined_df = pd.DataFrame(combined_data).sort_values("Total", ascending=False)
    ...
    top_party_name = combined_df.iloc[0]["party"] if not combined_df.empty else "N/A"
    st.markdown(
        f"""
        <div style="display: flex; justify-content: space-between; margin-top: 1rem; font-size: 0.95rem;">
            <span><strong>{party_symbol(top_party_name)}</strong> {top_party_name}: {top_total} seats</span>
            <span>Majority threshold: {MAJORITY_NEEDED}</span>
        </div>
...
def vs_builder_section(df):
    ...
    fptp_counts = declared["party"].value_counts().sort_values(ascending=False)
    parties = list(fptp_counts.index[:12])
    ...
    def get_pr(party):
        # Match PR input keys from majority section
        for i in range(10):
            key = f"pr_{party}_{i}"
    ...
    # FIXED: Proper DataFrame for Plotly
    ...
    fig = px.bar(
        ...
        color="Party",
        color_discrete_map={
            f"{party_symbol(party_a)} {party_a}": PARTY_COLOR_MAP.get(party_a, "#6B7280"),
            f"{party_symbol(party_b)} {party_b}": PARTY_COLOR_MAP.get(party_b, "#6B7280"),
        },
        text="Seats",
    )
    fig.update_layout(
    ...
    with col2:
        ...
        st.markdown(f"**{color_ab} A+B:** {combined} seats")
        need = max(MAJORITY_NEEDED - combined, 0)
        status = "✅ Majority secured" if combined >= MAJORITY_NEEDED else f"{need} seats short"
        st.markdown(f"**vs {MAJORITY_NEEDED}:** {status}")
...
def simple_party_totals(df):
    ...
    # FIXED: Proper DataFrame with named columns for Plotly
    party_data = []
    for party, count in declared["party"].value_counts().head(12).items():
        party_data.append({"Party": party, "Seats": count})
    party_df = pd.DataFrame(party_data).sort_values("Seats", ascending=True)
    ...
    fig = px.bar(
        ...
        text="Seats",
        color="Party",
        color_discrete_map={row["Party"]: PARTY_COLOR_MAP.get(row["Party"], "#6B7280") for _, row in party_df.iterrows()},
...
def main():
    ...
    st.markdown(
        """
        <div style="text-align: center; padding: 2rem; color: #94a3b8; font-size: 0.9rem; border-top: 1px solid rgba(255,255,255,0.1); margin-top: 3rem;">
